...
function App() {
  ...
  return (
    <>
      ...
      {roll ? (
        <video src={rolling} autoPlay></video>
      ) : (
        <div>
          <div className={isStart ? "container" : "notStarted"}>
            {isStart ? (
              <>
                <img src={dices[firstDiceNum]} />
                <img src={dices[secondDiceNum]} />
              </>
            ) : (
              <img src={notReady} />
            )}
          ...
          <button
            className={isStart ? "clickBox" : "startBox"}
            disabled={roll}
            onClick={continueGame}
          >
            <p>{isStart ? "add the score and continue" : "Start Game"}</p>
          </button>
          <button
            className={isStart ? "finishButton" : "hidden"}
            onClick={finishGame}
          >
            Finish the game
          </button>
      ...
      <span className="developer-name">Asliddin Choriyev™</span>
    </>
  );
}
